...
> **本次调整** (v1.1 → v1.2):
>   - 路线优化:**M5-M7 增加人形陪练敌人副线**(共用人形 Mannequin),为 M8 范式复用铺路
>   - 路径变更:D 盘只 3G 自由 → **UE 改装 F 盘**(`F:\UE_5.4\`,50G 自由,VS 2022 同盘)
>   - 风险备选新增 **次简版**(4-5 月,大型怪物简化到 4 招)
...
### 阶段 3:主角战斗系统 + 人形陪练副线(M5-M7,10 周)

**目标**:做出一个**可玩的主角 + 人形陪练战斗原型**(为 M8-M10 大型怪物范式复用铺路)
...
**副线任务**:(人形陪练敌人 — v1.2 新增,全程穿插共 1 周工作量)

> **为什么加人形的**:
> - UE 战斗系统(FSM + BT + 输入缓冲 + 反馈 + 镜头 + UI)在主角 + 人形陪练之间**完全同构**,M5-M7 阶段跑通后,M8-M10 大型怪物的工程范式 80% 可复用
> - **交付物升级**:M7 末交付"主角 vs 人形战斗 demo"(可演示)**+** M10 末再交付"主角 vs 大型怪物 demo"
> - **风险兜底**:如果大型怪物卡住了,M7 末已经有一个可演示人形 demo 兜底,**作品集不掉队**

5. **人形陪练敌人**(1 周工作量,穿插 M5-M6)
   - **角色**:UE Mannequin 改色(蓝衣红色 + 头盔占位)**复用 Mannequin 骨骼**,不需新模型
   - **招式表**:5-6 招够用(只要覆盖"近距单攻 / 中距突进 / 远程掷投"3 类即可)
     | 招式 | 距离 | 目的 |
     |---|---|---|
     | 横斩 | 近 | 基础攻击(等同轻击 1) |
     | 上挑 | 近 | 击飞(对应主角防御反馈测试) |
     | 冲刺斩 | 中 | 主动接近玩家 |
     | 投掷飞刀 | 远 | 远程骚扰,逼迫玩家走位 |
     | 后跳闪避 | 任意 | 玩家攻击时 20% 概率触发 |
     | 受击硬直 | 受击 | 验证战斗反馈时序 |
   - **FSM**(5 状态即可):Idle / Patrol / Chase / Attack / Hit / Death
   - **AI**(M5-M7 用最简 if-else 即可,**M8 再升级到 Behavior Tree**):
     ```
     if Distance(player) < 2m:
        if random() < 0.6: 横斩
        else: 上挑
     elif Distance(player) < 6m:
        冲刺斩
     else:
        巡逻 → 接近玩家 → 投掷飞刀
     ```
   - **HP + 死亡**:复用主角的 Attribute Component 思路
   - **验证目标**:主角 vs 人形陪练,完整打 1 局 3-5 分钟,验证整套战斗手感
...
- **副线策略**:人形陪练的招式表 / 状态机 / AI 决策,**全部填进同一个 excel 设计骨架**(主角招 / 怪物招 / 怪物状态 / AI 决策 sheet 直接复用),不新建独立 md
...
- `主角-状态机图.png`
- `主角-战斗反馈时序表.md`
- UE 项目:`主角战斗原型.uproject`(含主角 + 人形陪练)

---

### 阶段 4:大型怪物完整实现(M8-M10,8 周,基于 M5-M7 范式复用)

> **v1.2 新增说明**:
> M5-M7 已经跑通了"主角 + 人形陪练"完整战斗范式,工程代码(FSM 状态机模板 / BT 节点框架 / 输入缓冲 Component / 战斗反馈 Pipeline / 镜头和 UI)**复用率约 80%**。
> M8-M10 只多 3 个增量:体型放大(碰撞 + 导航 + 镜头) + 多部位 Hitbox(独立部位伤害) + 怪物专有阶段(疲劳 / 狂暴)。
> 不再涉及 UE 入门 / 引擎基础 / 范式搭建这些"已通过"的环节,工作量从原 8 周略微降至 6-8 周,留出 2 周缓冲。
...
3. 安装 UE 5.4 LTS(选 Windows,约 30 GB,**改装 F:\UE_5.4\**,D 盘只剩 3G 装不下)
4. 安装 Visual Studio 2022 Community(同步装 F 盘,选"使用 C++ 的游戏开发"工作负载,
   组件勾选:MSVC v143 / Windows 11 SDK / .NET 桌面开发,默认路径手工改 F:\VS2022\)
...
> **修订原因**:D 盘只剩 3G 自由空间,装不下 UE 5.4(~30G)。改装 F 盘(50G 自由)够用。
> E 盘(46.9G)和 F 盘(50G)都是备选,F 盘更大所以推荐 F。VS 2022 同盘,避免跨盘性能损耗。

```
F:\
├── UE_5.4\                       # UE 安装路径(避免 C 盘)
├── VS2022\                       # Visual Studio 2022 Community(手工选路径)
├── UE_Projects\                  # UE 项目根目录(每个项目独立 .gitignore)
│   └── LargeMonsterCombat\       # 大型怪物战斗 demo 项目
│       ├── Source\
│       ├── Content\
│       ├── Config\
│       └── *.uproject
├── portfolio\                    # 注意:portfolio 仍在 D 盘,文档同步走 git
│   ├── 大型怪物战斗\
...
**备选**(v1.2 修订):
- **全量版**(v1.2 默认,6 个月):MH 拆解 + 主角 + 人形陪练完整 + 大型怪物(8 状态 8 招)完整实现 ⭐
- **次简版**(4-5 个月):MH 拆解 + 主角 + 人形陪练完整 + **简化大型怪物(只做 4 招,不做疲劳/狂暴)**
- **极简版**(3-4 个月):MH 拆解 + 主角 + **简化人形陪练**(无 BT,只用 if-else,3 招)**最低限度完工**
...
- v1.1 (2026-07-02):**修订**:
  ...
  - 6 难点改为 **UE 特定方案**
- v1.2 (2026-07-08):**修订**(Mavis 摸底后):
  - **M5-M7 增加人形陪练敌人副线**(共用人形 Mannequin,5-6 招,FSM 5 状态)
  - **M8-M10 大型怪物实现工作量从原 8 周降至 6-8 周**(因为 M5-M7 已通范式)
  - **路径 D:\UE_5.4\ → F:\UE_5.4\**(D 盘只剩 3G 装不下,改装 F 盘 50G 自由)
  - **VS 2022 同步装 F 盘**
  - **风险备选新增 次简版**(4-5 月,大型怪物简化到 4 招)
  - 第十节补充 **excel 设计骨架**(10 sheet,80% 范式复用到大型怪物)
